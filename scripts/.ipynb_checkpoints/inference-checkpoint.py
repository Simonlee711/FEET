...
import random
from sklearn.model_selection import train_test_split
from scipy import stats  # Added import for statistical computations
    ...
    def benchmark(self, batch_size=32):
        ...
        for _ in range(n_bootstraps):
            ...
            if len(np.unique(all_labels[indices])) < 2:
                continue
            accuracy_scores.append(accuracy_score(all_labels[indices], all_preds[indices]))
            precision_scores.append(precision_score(all_labels[indices], all_preds[indices], average='weighted', zero_division=0))
            recall_scores.append(recall_score(all_labels[indices], all_preds[indices], average='weighted', zero_division=0))
            f1_scores.append(f1_score(all_labels[indices], all_preds[indices], average='weighted', zero_division=0))
...
class ModelFewShotLearner:
    ...
    def few_shot_learning(self, max_N=10, batch_size=8, num_epochs=3, validation_split=0.1):
        ...
        for n in range(1, max_N + 1):
            ...
            results[sample_size] = eval_result
    ...
    def compute_metrics(self, pred):
        """
        Compute metrics for evaluation, including confidence intervals.
        """
